...
// label counter for creating unique labels/data names
//int label_count = 0;
...
static void build_basic_blocks(TacInstr* tac_head) {
    ...
    // Pass 1: map instructions by index and find leaders using indices (safe)
    int idx = 0;
    for (TacInstr* instr = tac_head; instr; instr = instr->next, idx++) {
        instr_map[idx] = instr;
        if (idx == 0) {
            leaders[idx] = true; // First instruction is a leader
        }

        if (instr->op == TAC_LABEL || instr->op == TAC_BEGIN_FUNC) {
            leaders[idx] = true;
        }
        if (instr->prev) {
...
static void compute_liveness(BasicBlock* block, List* vars_in_func) {
    ...
    // Backward pass: process from end down to start, inclusive
    ...
    for (;;) {
        ...
        if (instr->res) {
            ...
            } else if (instr->res->kind == ADDR_TEMP && temp_live_info) {
        ...
        if (instr->arg1) {
            ...
            } else if (instr->arg1->kind == ADDR_TEMP && temp_live_info) {
        ...
        if (instr == block->start) break;
        instr = instr->prev;
        if (!instr) break; // safety guard
    }
}
...
static void mips_emit_data_segment(TacInstr* tac_head) {
    ...
    // Add 0.0 float constant for comparisons
...
static void mips_emit_text_segment() {
    ...
    // Load 0.0 into its dedicated register using canonical lwc1
    mips_emit("lwc1 %s, f_const_zero", TEMP_FPR_ZERO);
}

static void mips_emit_prologue(Symbol* func_sym) {
    mips_emit("\n# --- Function Prologue: %s ---", func_sym->name);
    mips_emit("%s:", func_sym->name);
...
static int find_gpr_to_spill(int exclude_reg) {
    ...
    for (int i = 0; i < NUM_TEMP_REGS; i++) {
        ...
        for (ListNode* n = list_begin(vars_in_reg); n; n = list_next(n)) {
            ...
            if (is_live) {
                all_dead = false;
                if (next_use == -1) next_use = 999999;
                if (next_use > reg_max_next_use) reg_max_next_use = next_use;
            }
        }
...
static const char* getFPR(TacInstr* instr, TacAddr* var, bool is_res, int exclude_reg_idx) {
    ...
    for (ListNode* n = list_begin(ad->locations); n; n = list_next(n)) {
        AddrLocation* loc = (AddrLocation*)list_get_data(n);
        if (loc->kind == LOC_FPR) {
            return float_reg_names[loc->reg_index];
        }
    }
