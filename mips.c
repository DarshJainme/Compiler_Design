...
static void ad_remove_reg_loc(TacAddr* var, int reg_index) {
    AddrDesc* ad = get_or_create_ad_entry(var);
    ListNode* n = list_begin(ad->locations);
    while (n) {
        AddrLocation* loc = (AddrLocation*)list_get_data(n);
        if (loc->kind == LOC_REG && loc->reg_index == reg_index) {
            list_remove(ad->locations, loc);
            free(loc);
            break; // Assuming var is in a reg at most once
        }
        n = list_next(n);
    }
}
...
static void free_dead_regs(TacInstr* instr) {
    ...
    for (int i=0; i<2; i++) {
        ...
        if (var->kind == ADDR_VARIABLE && var->val.var) {
            ...
            is_live = false; // Constants are always "dead" (don't occupy regs)
        }
        
        // A var is dead if it's not live, OR if its next use is this line
        if (!is_live || next_use == current_line) {
            // Find the register it's in and clear it
            AddrDesc* ad = get_or_create_ad_entry(var);
            ListNode* n = list_begin(ad->locations);
            while (n) {
                AddrLocation* loc = (AddrLocation*)list_get_data(n);
                if (loc->kind == LOC_REG) {
                    rd_remove_var_from_reg(var, loc->reg_index);
                    // Don't remove from AD, just remove this reg location
                    list_remove(ad->locations, loc);
                    free(loc);
                    break; // Move to next var
                }
                n = list_next(n);
            }
        }
...
static void gen_mips_for_instr(TacInstr* instr, BasicBlock* block) {
    ...
    // mips_comment("3AC: %s", "..."); // HACK: We need a print_tac_instr()
    
    switch(instr->op) {
        // --- Function Start/End ---
        case TAC_BEGIN_FUNC:
            // Stack offset calculation is now done in generate_mips loop
            mips_emit_prologue(instr->res->val.var);
            break;
        case TAC_END_FUNC:
            // This is just a marker. The epilogue is generated
            // by the TAC_RETURN instruction or at the end.
        ...
        case TAC_IFZ: // if arg1 == 0 goto label
            r_arg1 = get_reg(instr, instr->arg1, false);
        ...
        case TAC_ASSIGN: // res = arg1
            r_arg1 = get_reg(instr, instr->arg1, false);
            r_res = get_reg(instr, instr->res, true); // Get reg for res, don't load
        ...
        case TAC_ADD: // res = arg1 + arg2
            r_arg1 = get_reg(instr, instr->arg1, false);
            r_arg2 = get_reg(instr, instr->arg2, false);
            r_res = get_reg(instr, instr->res, true); 
            mips_emit("addu %s, %s, %s", r_res, r_arg1, r_arg2); // Use 'addu'
            free_dead_regs(instr);
            break;
        ...
        // ... (Relational Ops are similar, use get_reg(..., false)) ...
        case TAC_LT:
            r_arg1 = get_reg(instr, instr->arg1, false);
            r_arg2 = get_reg(instr, instr->arg2, false);
            r_res = get_reg(instr, instr->res, true);
        ...
        // --- Function Calls ---
        case TAC_PARAM:
            // A better param implementation:
            // 1. Count params backwards from TAC_CALL
            // 2. Load first 4 into $a0-$a3
            // 3. Push rest onto stack
            // For now, we stick to the simple (but slow) stack-only version.
            r_arg1 = get_reg(instr, instr->arg1, false); // Get value into a reg
            mips_emit("subu $sp, $sp, 4"); // Make space
            mips_emit("sw %s, 0($sp)", r_arg1); // Push param
            free_dead_regs(instr);
            break;
        ...
        case TAC_CALL:
            ...
            // 3. Get return value (by convention, in $v0)
            if (instr->res) {
                r_res = get_reg(instr, instr->res, true);
        ...
        case TAC_RETURN:
            if (instr->arg1) {
                r_arg1 = get_reg(instr, instr->arg1, false);
                mips_emit("move $v0, %s", r_arg1); // Put return val in $v0
            }
            // Find the function symbol to jump to the epilogue
            Symbol* func_sym = block->start->res->val.var; // Assumes block starts with BEGIN_FUNC
        ...
        case TAC_DEREF: // res = *arg1 (load)
            r_arg1 = get_reg(instr, instr->arg1, false); // Get pointer value
            r_res = get_reg(instr, instr->res, true);   // Get reg for result
            mips_emit("lw %s, 0(%s)", r_res, r_arg1);
            free_dead_regs(instr);
            break;
            
        case TAC_STORE: // *res = arg1 (store)
            r_res = get_reg(instr, instr->res, false);  // Get address
            r_arg1 = get_reg(instr, instr->arg1, false); // Get value
...
static void store_regs_at_block_end(BasicBlock* block) {
    ...
    // Check if the last instruction is a jump/return
    ...
    for (int i = 0; i < NUM_TEMP_REGS; i++) {
        ...
        // If it's a block exit, spill everything
        if (is_exit) {
            spill_reg(i);
        } else {
            // Otherwise, only spill variables that are live-out
            for (ListNode* n = list_begin(vars_in_reg); n; n = list_next(n)) {
                TacAddr* var = (TacAddr*)list_get_data(n);
                bool is_live = false;
                if (var->kind == ADDR_VARIABLE && var->val.var) {
...
void generate_mips(TacInstr* tac_head, const char* out_filename) {
    ...
    // 1. Emit Data Segment (Strings, Globals)
    mips_emit_data_segment(tac_head);

    // 2. Emit Text Segment
    mips_emit_text_segment();
    ...
    for (BasicBlock* block = block_list_head; block; block = block->next) {
        ...
        // 8. At block end, store any live vars still in registers
        store_regs_at_block_end(block);
    }
    
    // Free lists from the last function
    if (current_func_vars) list_destroy(current_func_vars);
    if (current_func_temps) list_destroy(current_func_temps);
